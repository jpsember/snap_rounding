...
// TESTBED: make this part of the testbed package.
package testbed;
 ...
 *
 */
  class EpsGraphics2D    extends java.awt.Graphics2D {
   ...
   * @deprecated
   */
  private EpsGraphics2D(String title, OutputStream outputStream, int minX,                       int minY, int maxX, int maxY) throws IOException {
    this(title);
...
//      // TESTBED: define bounding rectangle if not already defined
//      if (lb == null) {
//        lb = s.getBounds2D();
//      }
      ...
      // TESTBED: don't update bounds dynamically if using static bounds
//       if (!fixedBoundsFlag)
      {
        // Update the bounds.
...
//          count++;
  ...
  public void setStroke(Stroke s) {
    if (s instanceof BasicStroke) {
      ...
      // TESTBED:  EPS strokes seem thinner than Java 2D ones.
      final float STROKE_ADJUST = 4 * 1.15f;
      if (false) {
        append(_stroke.getLineWidth() * STROKE_ADJUST + " setlinewidth");
      } else
      ...
      if (dashArray != null) {
        for (int i = 0; i < dashArray.length; i++) {
          // TESTBED: EPS stroke adjustment
          if (false)
          dashes.append( (dashArray[i] * STROKE_ADJUST) + " ");
        else
...
//    System.err.println(getColorDepth());
  ...
  // TESTBED: support for setting static bounds

//  private boolean fixedBoundsFlag = false;

//  public void setFixedBounds(FRect r) {
//    fixedBoundsFlag = true;
//
//    _document.updateBounds( (float) r.getX(), (float) - r.getY());
//    _document.updateBounds( (float) (r.endX()),
//                           (float) ( -r.endY()));
//  }

  public void setMaxBounds(FRect r) {
    _document.setMaxBounds(new FRect(r.x,-r.endY(),r.width, r.height));
  }
}
